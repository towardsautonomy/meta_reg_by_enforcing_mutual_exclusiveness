...
def main(argv):
  ...
  for i, sampled_folders in enumerate(folders):
    print('Loading metatrain [{}/{}]'.format(i+1, len(folders)))
  ...
  for i, sampled_folders in enumerate(folders):
    print('Loading metaval [{}/{}]'.format(i+1, len(folders)))
    if os.listdir(sampled_folders):
      labels_and_images = get_images(sampled_folders, shuffle=False)
  ...
  for i, sampled_folders in enumerate(folders):
    print('Loading metatest [{}/{}]'.format(i+1, len(folders)))
    if os.listdir(sampled_folders):
      labels_and_images = get_images(sampled_folders, shuffle=False)
  ...
  with open(data_folder+'/test_data.pkl', 'wb') as f:
    pickle.dump([all_images, all_labels], f)
